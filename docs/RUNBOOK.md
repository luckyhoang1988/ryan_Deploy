...
Tạo một service account trong AD, VD `CORP\svc_pydeploy`:
- **KHÔNG dùng Domain Admin.** Chỉ cần là **local admin trên máy trạm**.
- Cấp local admin qua GPO → *Restricted Groups* hoặc *Group Policy Preferences → Local Users and Groups*, thêm `svc_pydeploy` vào nhóm `Administrators` của các máy đích.
- Đặt mật khẩu mạnh, bật "password never expires" hoặc có quy trình xoay vòng.

### 1.2. Mở đường mạng tới máy trạm
Trên máy trạm (qua GPO Firewall), cho phép từ IP server PyDeploy:
...
net use \\PC-LAB-01\ADMIN$ /user:CORP\svc_pydeploy
```
Nếu map được → PyDeploy sẽ đẩy được.
...
- `PYDEPLOY_VAULT_KEY` — khóa mã hóa credential (BẮT BUỘC, không được đổi sau khi đã lưu credential).
- `PYDEPLOY_MAX_CONCURRENCY` — số máy đẩy song song (mặc định 15).
...
2. **Nạp credential**: tạo `DeployCredential` (chỉ admin) — trỏ tới `svc_pydeploy`.
...
- **PostgreSQL**: `docker compose exec postgres pg_dump -U pydeploy pydeploy > backup.sql` (định kỳ).
